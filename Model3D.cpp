...
namespace gps {
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
                    ...
                    //colis
                    setX(vertexPosition.x);
                    setY(vertexPosition.y);
                    setZ(vertexPosition.z);
                    //std::cout<<" "<<getX();
    ...
    glm::vec3 Model3D:: CalculateAxismin()
    {
        std::vector<gps::Vertex> vertices;

        glm::vec3  max( -100, 0, 0);
        glm::vec3  min(999, 1, 1);
    ...
    glm::vec3 Model3D:: CalculateAxisMax()
    {
        std::vector<gps::Vertex> vertices;
